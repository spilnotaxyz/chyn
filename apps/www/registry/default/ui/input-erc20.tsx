...
>(
  (
    ...
    return (
      <div className={cn("flex justify-between py-2 px-3 h-10 text-sm rounded-md border border-input bg-background ring-offset-background has-[:focus-visible]:outline-none has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring has-[:focus-visible]:ring-offset-2 has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-50",className)}>
      <input
        type="number"
        className={cn(
          "[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none bg-background placeholder:text-muted-foreground focus-visible:outline-none grow",
          inputClassName
        )}
        ref={ref}
        onChange={(e) => {
      ...
      />
        ...
        <span className="text-sm select-none text-muted-foreground">{token.symbol}</span>
        </div>
</div>
    )
  }
...
>(({ className, inputClassName, ...props }, ref) => {
  return (
      ...
      <div id="pseudo-input" className={cn("flex justify-between py-2 px-3 h-10 text-sm rounded-md border border-input bg-background ring-offset-background has-[:focus-visible]:outline-none has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring has-[:focus-visible]:ring-offset-2 has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-50", className )}>
      <input
        type="number"
        className={cn(
          "[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none bg-background placeholder:text-muted-foreground focus-visible:outline-none grow",
...
> &
  GetTokenOptions<config> & {
    value?: bigint
    onChange?: (e: React.ChangeEvent<HTMLInputElement>, parsedValue: bigint) => void
...
const InputERC20 = React.forwardRef<HTMLInputElement, InputERC20Props>(
  (props, ref) => (
    <React.Suspense fallback={<InputERC20Skeleton placeholder={props.placeholder} />}>

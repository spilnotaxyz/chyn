...
Run the `chyn` init command to setup your project:

```bash
npx chyn@latest init
```
...
When running `npx chyn@latest init`, your tailwind config for content will be overwritten. To fix this, change the `module.exports` to `export default` and the `content` section with the code below to your `tailwind.config.mjs` file:
